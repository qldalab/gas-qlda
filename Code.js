...
function doGet(e) {
  return HtmlService.createTemplateFromFile('index')
      ...
      .setFaviconUrl("https://static.vecteezy.com/system/resources/thumbnails/046/680/406/small/3d-report-icon-report-symbol-3d-free-png.png");
}
...
function addProject(projectData) {
  ...
  try {
    ...
    const projectSheet = getOrCreateSheet(ss, PROJECT_SHEET_NAME, [
      ...
      PROJECT_TASKS_JSON_COLUMN_NAME, PROJECT_ACTIVITY_LOG_JSON_COLUMN_NAME
    ]);
...
function addTask(taskData) {
  ...
  try {
    ...
    if (jsonColIndex === -1) throw new Error(`Không tìm thấy cột "${PROJECT_TASKS_JSON_COLUMN_NAME}".`);
    ...
    newTask[TASK_DESC_COLUMN_NAME] = taskData.description ? String(taskData.description).trim() : '';
    ...
    newTask[TASK_RESULT_LINKS_COLUMN_NAME] = taskData.resultLinks ? String(taskData.resultLinks).trim() : '';
    ...
    logActivity("Thêm nhiệm vụ", `Tên: ${taskData.name}, Giao cho: ${taskData.assignee || 'N/A'}, ID: ${newTaskId}`, taskData.projectId);
...
function updateTask(taskId, taskData) {
  ...
  try {
    ...
    if (oldProjectId !== taskData.projectId) {
      ...
      logActivity("Chuyển nhiệm vụ", `ID: ${taskId} → ${newTaskId}, Tên: ${taskData.name}, Từ ${oldProjectId} sang ${taskData.projectId}`, taskData.projectId);
    ...
    updatedTask[TASK_DESC_COLUMN_NAME] = taskData.description ? String(taskData.description).trim() : '';
    updatedTask[TASK_ASSIGNEE_COLUMN_NAME] = taskData.assignee ? String(taskData.assignee).trim() : '';
    ...
    updatedTask[TASK_RESULT_LINKS_COLUMN_NAME] = taskData.resultLinks ? String(taskData.resultLinks).trim() : '';
      ...
      try {
        ...
        if (jsonStr && typeof jsonStr === 'string') {
          ...
          if (taskIndex !== -1) {
            ...
            logActivity("Xóa nhiệm vụ", `ID: ${taskId}, Tên: ${taskName}`, originalTask[TASK_PROJECT_ID_COLUMN_NAME]);
            return { success: true };
          }
        }
      } catch (e) {
...
function copyProject(projectId, newProjectName) {
  ...
  try {
    ...
    logActivity("Tạo bản sao dự án", `ID gốc: ${projectId}, ID mới: ${newProjectId}, Tên: ${newProjectName}`, newProjectId);
...
function copyTask(taskId, newTaskName) {
  ...
  try {
    ...
    logActivity("Tạo bản sao nhiệm vụ", `ID gốc: ${taskId}, ID mới: ${newTaskId}, Tên: ${newTaskName}`, originalTask[TASK_PROJECT_ID_COLUMN_NAME]);
...
function getChatMessages() {
  try {
    ...
    allMessages.sort((a, b) => new Date(a.chatDate + ' ' + a.timestamp) - new Date(b.chatDate + ' ' + b.timestamp));
...
function sendChatMessage(message) {
  ...
  try {
    ...
    const chatSheet = getOrCreateSheet(ss, CHAT_SHEET_NAME, [
      CHAT_ID_COLUMN_NAME, CHAT_DATE_COLUMN_NAME, CHAT_JSON_COLUMN_NAME
    ]);
...
function reorderTasks(projectId, orderedTaskIds) {
  ...
  try {
    ...
    } catch (e) { return { success: false, error: "Lỗi dữ liệu JSON" }; }
